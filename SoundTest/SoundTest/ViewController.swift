...
//  SoundTest
//
//  Created by Hanbyeol Yoo on 17/01/2019.
//  Copyright © 2019 yhb4ever. All rights reserved.
//
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "AlertSoundTableViewCell", for: indexPath)
        cell.textLabel!.text = String(soundData[indexPath.row])
        return cell
    }
}
...
extension ViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        AudioServicesPlayAlertSound(SystemSoundID(soundData[indexPath.row]))
    }
}
